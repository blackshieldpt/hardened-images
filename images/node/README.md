...
Wolfi-based hardened Node.js image, assembled with apko from the Wolfi `nodejs-22` package.

## Details

| Property   | Value |
|------------|-------|
| Build      | apko (Wolfi nodejs-22, npm) |
| Version    | 22 |
...
```bash
docker run --rm -v $(pwd):/app -w /app hub.blackshield.pt/test_images/node:22 index.js
```
...
```dockerfile
FROM hub.blackshield.pt/test_images/node:22-dev AS build
WORKDIR /app
COPY package*.json ./
RUN npm ci
COPY . .
...
FROM hub.blackshield.pt/test_images/node:22
WORKDIR /app
...
```bash
docker run -it --entrypoint /bin/sh hub.blackshield.pt/test_images/node:latest-dev
```
